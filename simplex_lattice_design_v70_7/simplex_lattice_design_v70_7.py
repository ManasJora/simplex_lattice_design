def simplex_lattice_design_v70_7():
    r"""
    # Overview
    ...
    def calculate_design(component_data, degree_m, total_formula_mass, replicate, randomize):
        ...
        for p_tuple in itertools.product(range(degree_m + 1), repeat=n_components):
            if sum(p_tuple) == degree_m:
                ...
                # --- STEP 1: Calculate Initial Product Masses based on Lattice & Active Limits ---
                # Note: We calculate for ALL components here initially
                current_sum_mass = 0.0
                calc_names_for_norm = [] # Names to be used in normalization if no solvent

                if solvent_component_name:
                    # Scenario A: With Solvent
                    # Calculate active ingredients first
                    ...
                    # Calculate Solvent by Difference
                    current_actives_mass = sum(temp_product_masses.values())
                    req_solvent = total_formula_mass - current_actives_mass
                    ...
                    # Scenario B: No Solvent (Pure Mixture)
                    # Calculate product mass for everyone based on constraints
                    ...
                    # --- NORMALIZATION LOGIC ---
                    # Sum current masses
                    current_total = sum(temp_product_masses.values())
                    
                    # Normalize to Total Formula Mass
                    if current_total > 0:
                        normalization_factor = total_formula_mass / current_total
                        for name in component_names:
                            temp_product_masses[name] = temp_product_masses[name] * normalization_factor
                    else:
                        # Edge case: sum is 0 (shouldn't happen in standard lattice), distributes equally or keeps 0
                        pass 
                ...
                # Validation limits (Strict 1% tolerance)
                if final_sum_mass > (total_formula_mass * 1.01):
                    reason, is_valid = "Sum(Product) > Total Mass", False
                elif final_sum_mass < (total_formula_mass * 0.99):
                     # If normalization worked or solvent worked, this should be close to Total Mass.
                     # If it's still low, something is wrong (e.g. negative solvent required)
                     if solvent_component_name and temp_product_masses[solvent_component_name] < 0:
                         reason, is_valid = "Negative Solvent Required", False
                     elif not solvent_component_name:
                         # Should not happen with normalization unless total mass is 0
                         pass
                ...
                for name in component_names:
                    ...
                    if name == solvent_component_name: 
                        a_mass = total_solvent_active
                    else: 
                        a_mass = p_mass * product_purity_map[name]
        ...
        return df_valid, df_removed, colors_hex, component_names, final_cols, solvent_component_name
    ...
    # --- Widgets ---
    w_degree = widgets.BoundedIntText(value=3, min=1, max=15, description='Degree (m):', style={'description_width': 'initial'}, layout=widgets.Layout(width='160px'))
    ...
    w_reps = widgets.BoundedIntText(value=1, min=1, max=10, description='Replicates:', style={'description_width': 'initial'}, layout=widgets.Layout(width='160px'))
    ...
    def add_ingredient_row(b=None, initial_data=None):
        ...
        w_purity = widgets.BoundedFloatText(value=data['purity'], min=0.0, max=100.0, step=1.0, description='Purity (%):', 
                                            layout=widgets.Layout(width='130px'), style={'description_width': '70px'})
        
        w_max = widgets.BoundedFloatText(value=data['max'], min=0.0, max=100.0, step=1.0, description='Max Active (%):', 
                                         layout=widgets.Layout(width='170px'), style={'description_width': '110px'})
    ...
    def on_run(b):
        ...
        results_widgets = []
        results_widgets.append(widgets.HTML("<b style='color:black; font-size:16px;'>--- Running Simplex Lattice Design v70.7 ---</b>"))

        try:
            df_valid, df_removed, colors_hex, component_names, final_cols, solv_name = calculate_design(
                comp_data, w_degree.value, w_mass.value, w_reps.value, w_rand.value
            )
            
            # Show Detected Solvent
            if solv_name:
                results_widgets.append(widgets.HTML(f"<b>Solvent Used:</b> {solv_name}"))
            else:
                results_widgets.append(widgets.HTML("<b>Solvent Used:</b> None (Pure Mixture - Masses Normalized)"))
            ...
            # 4. EXPORT Logic (Modified to Excel with Parameters)
            if w_csv.value and not df_valid.empty:
                fname = f'simplex_lattice_design_v70_7_{datetime.now().strftime("%H%M%S")}.xlsx'
                
                # Check for openpyxl
                import importlib.util
                if importlib.util.find_spec("openpyxl") is None:
                     print("❌ Error: 'openpyxl' library is required for Excel export. Please install it (pip install openpyxl).")
                else:
                    ...
                    # Add Ingredients to params
                    params_data.append(['', '']) # Spacer
                    params_data.append(['Ingredients Config', ''])
                    params_data.append(['Name', 'Purity (%) | Max (%) | Dens | Is Solvent'])
                    ...
                    print(f"✅ Excel Exported: {fname} (Sheets: Datapoints, Parameters)")
    ...
    ui_content = [
        widgets.HTML("<div class='header-text'>🧪 Simplex Lattice Design v70.7</div>"),
